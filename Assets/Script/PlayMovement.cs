...
public class PlayMovement : MonoBehaviour
{
    ...
    public float coyoteTimerCounter;                                     //관성 타이머
    ...
    void Update()
    {
        ...
        // G키로 글라이더 제어 (누르는 동안 활성화)
         if (Input.GetKey(KeyCode.G) && !isGrounded && gliderTimerLeft > 0)  // G키를 누르면서 땅에 있지 않고 글라이더 남은 시간이 있을 때 (3가지 조건)
        {
            ...
            {
                DisableGlider();                                                 // G 키를 떼면 글라이더 비활성화
            }
            ...
            else
            {
                ...
                {
                    rb.linearVelocity += Vector3.up * Physics.gravity.y * (fallMutiplier - 1) * Time.deltaTime;    // 하가 시 중력 강화
                }
        ...
        if (rb.linearVelocity.y < 0)
        {
            rb.linearVelocity += Vector3.up * Physics.gravity.y * (fallMutiplier - 1) * Time.deltaTime;      //하가 시 중력 강와
        }
        ...
        //점프 입력
        if (Input.GetButtonDown("Jump") && isGrounded)  // && 두 값을 만족할 때 -> (스페이스 버튼을 눌렀을때와 땅위에 있을때)
        {
        ...
        gliderTimerLeft = gliderMaxTime;              // 지상에 있을 때 시간 회복 
    }
    ...
    void OnCollisionEnter(Collision collision)                       //유니티에서 지원해주는 충돌 체크 함수 
    {
        ...
        {
            realGrouned = true;                        //땅이 충돌하면 true로 만들어 준다.
        }
    }

    void OnCollisionStay(Collision collision)                      //지면과의 충돌이 유지 되는지 확인
    {
        if (collision.gameObject.tag == "Ground")                    //충동이 일어난 물체의 Tag가 Ground인 경우
        {
    ...
    void UpdateGrounededState()
    {
        ...
        {
            //실제로는 지면이 없지만 코요테 타임  내에 있으면 여전히 지면으로 판단
            if (coyoteTimerCounter > 0)
            {
                coyoteTimerCounter -= Time.deltaTime;                       //시간을 지속적으로 감소 시킨다.
                isGrounded = true;
            }
            else
            {
    ...
    {
        // 그라이더 효과 : 천천히  떨어지고 수평 방향으로 빠르게 이동
